...
//   } else {
//     res.status(400).send(
//         'Invalid data. Ensure "name" and "tests" are present in the request
//         body.');
//   }
// });
...
app.post('/', (req, res) => {
  ...
  if (req.body && req.body.name && req.body.tests) {
    ...
    // Write the tests to an input file
    const inputFilePath = path.join(OUTPUT_DIR, inputFileName);
    const testsContent = req.body.tests.map(test => test.input).join('\n');
    fs.writeFileSync(inputFilePath, testsContent, 'utf8');
